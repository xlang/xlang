...
xlang-js
 * license at http://xlang/xlang.js.
 *
 * @author      XiaoLang.X.Q<xlangcn@126.com>
 * @copyright   2005-2014 XiaoLang.
 * @version     SVN: $Id: Xlang.js 20M 2014-04-29 04:26:23Z (local) $
 */

/**
 * The Xlangdowbox class. Used to display different media on a web page using a
 ...
 */
var Xlangdowbox = function(){

    var ua = navigator.userAgent.toLowerCase(),

    // the Xlangdowbox object
    S = {

        /**
         * The current version of Xlangdowbox.
         *
         * @var     String
         * @public
         */
        ...
        /**
         * Holds the current dimensions of Xlangdowbox as calculated by its skin.
         * Contains the following properties:
         *
         * - height: The total height of #Xl-wrapper (including title & info bars)
         * - width: The total width of #Xl-wrapper
         * - inner_h: The height of #Xl-body
         * - inner_w: The width of #Xl-body
         * - top: The top to use for #Xl-wrapper
         * - left: The left to use for #Xl-wrapper
        ...
        /**
         * Contains the base path of the Xlangdowbox script.
         *
         * Note: This property will automatically be populated in Xlangdowbox.load.
         *
         * @var     String
         * @public
         */
        ...
        /**
         * Contains the default options for Xlangdowbox.
         *
         * @var     Object
         * @public
         */
        options: {
             ...
             * default. Also, any modifications must be passed into Xlangdowbox.init
             * for speed reasons.
             */
            ext: {
             ...
             * its original resolution but it will be draggable within Xlangdowbox. A
            ...
            modal: false,               // trigger Xlangdowbox.close() when overlay is
                                        // clicked
            ...
            skipSetup: false,           // skip calling Xlangdowbox.setup() during
                                        // Xlangdowbox.init()
        ...
        /**
         * A map of library object names to their corresponding Xlangdowbox adapter
         * names.
         *
         * @var     Object
         * @public
         ...
         *
         * Note: Options will be reset on Xlangdowbox.open() so this function is
         * only useful after it has already been called (while Xlangdowbox is
         * open).
         *
        ...
        buildCacheObj: function(link, opts){
            ...
            if(rel){
                // extract gallery name from Xlangdowbox[name] format
         ...
         * Xlangdowbox and clears all objects from cache.
         *
         * @return  void
         * @public
         */
        ...
        /**
         * Initializes the Xlangdowbox environment. Should be called by the user in
         * the <head> of the HTML document.
         *
         * Note: This function attempts to load all Xlangdowbox dependencies
        ...
        init: function(opts){
            ...
            if(!S.lang)
                U.include(S.path + 'languages/Xlangdowbox-' + S.options.language + '.js');
            each(S.options.players, function(p){
                if((p == 'swf' || p == 'flv') && !window['swfobject'])
                    U.include(S.path + 'libraries/swfobject/swfobject.js');
                if(!S[p])
                    U.include(S.path + 'players/Xlangdowbox-' + p + '.js');
            });
            if(!S.lib)
                U.include(S.path + 'adapters/Xlangdowbox-' + S.adapter + '.js');
        },

        /**
         * Tells whether or not Xlangdowbox is currently activated.
         *
        ...
        /**
         * Tells whether or not Xlangdowbox is currently in the middle of a
        ...
        /**
         * Loads Xlangdowbox into the DOM. Is called automatically by each adapter
        ...
        /**
         * Opens the given object in Xlangdowbox. This object may be either an
         * anchor/area element, or an object similar to the one created by
         * Xlangdowbox.buildCacheObj().
         *
            ...
            for(var i = 0; i < S.gallery.length; ++i){
                ...
                if(g.player == 'unsupported'){
                    ...
                    if(S.options.handleUnsupported == 'link'){
                        ...
                        g.content = '<div class="Xl-message">' + msg + '</div>';
                    }else
                        r = true;
        ...
        /**
         * Reverts Xlangdowbox' options to the last default set in use before
         * Xlangdowbox.applyOptions() was called.
         *
         * @return  void
         * @public
         */
        ...
        /**
         * Calculates the dimensions for Xlangdowbox according to the given
         ...
         * @return  Object                  The Xlangdowbox.dimensions object
         * @public
         */
        ...
        /**
         * Sets up listeners on the given links that will trigger Xlangdowbox. If no
        ...
        setup: function(links, opts){
            ...
            each(links, function(link){
                if(typeof link.XlangdowboxCacheKey == 'undefined'){
                    // assign cache key expando, use integer primitive to avoid
                    // memory leak in IE
                    link.XlangdowboxCacheKey = S.cache.length;
                    // add onclick listener
                    S.lib.addEvent(link, 'click', handleClick);
                }
                S.cache[link.XlangdowboxCacheKey] = S.buildCacheObj(link, opts);
            });
        }
    ...
    /**
     * Keeps track of whether or not Xlangdowbox.init has been called.
     *
     * @var     Boolean
     * @private
     */
     ...
     * @private
     */
    content_id = 'Xl-content',

    /**
     * Keeps track of whether or not Xlangdowbox is activated.
     *
     * @var     Boolean
     * @private
     */
    ...
    /**
     * Handles all clicks on links that have been set up to work with Xlangdowbox
    ...
    function handleClick(e){
        ...
        if(link){
            ...
            var key = link.XlangdowboxCacheKey;
    ...
    /**
     * Loads the Xlangdowbox with the current piece.
     *
     * @return  void
     * @private
     */
...
/**
 * The default skin for Xlangdowbox. Separated out into its own class so that it may
 * be customized more easily by skin developers.
 */
Xlangdowbox.skin = function(){
     ...
     */
    function fixTop(){
        U.get('Xl-container').style.top = document.documentElement.scrollTop + 'px';
    }

    /**
     * Toggles the visibility of #Xl-container and sets its size (if on
    ...
    function toggleVisible(cb){
        var so = U.get('Xl-overlay'),
            sc = U.get('Xl-container'),
            Xl = U.get('Xl-wrapper');
        ...
        if(cb){
            ...
            if(S.options.showOverlay){
                ...
                Xl.style.display = 'none'; // cleared in onLoad
            }
    ...
    function buildBars(cb){
        ...
        U.get('Xl-title-inner').innerHTML = obj.title || '';
        ...
        if(S.options.displayCounter && S.gallery.length > 1){
            ...
            if(typeof count == 'string') // default
                ...
                U.each(count, function(i){
                    c += '<a onclick="Xlangdowbox.change(' + i + ');"'
                    if(i == S.current) c += ' class="Xl-counter-current"';
                    c += '>' + (i + 1) + '</a>';
                });
            }
        }
        U.get('Xl-counter').innerHTML = c;
    ...
    function hideBars(anim, cb){
        var sw = U.get('Xl-wrapper'),
            st = U.get('Xl-title'),
            si = U.get('Xl-info'),
            ti = U.get('Xl-title-inner'),
            ii = U.get('Xl-info-inner'),
    ...
    /**
     * Adjusts the height of #Xl-body and centers #Xl-wrapper vertically
     * in the viewport.
     *
     * @param   Number      height      The height to use for #Xl-body
     * @param   Number      top         The top to use for #Xl-wrapper
     * @param   Boolean     anim        True to animate the transition
    ...
    function adjustHeight(height, top, anim, cb){
        var Xl = U.get('Xl-body'),
            s = U.get('Xl-wrapper'),
            h = parseInt(height),
            t = parseInt(top);

        if(anim){
            U.animate(Xl, 'height', h, S.options.resizeDuration);
            U.animate(s, 'top', t, S.options.resizeDuration, cb);
        }else{
            Xl.style.height = h + 'px';
            s.style.top = t + 'px';
            if(cb) cb();
        }
    }
    ...
    /**
     * Adjusts the width and left of #Xl-wrapper.
     *
     * @param   Number      width       The width to use for #Xl-wrapper
     * @param   Number      left        The left to use for #Xl-wrapper
     * @param   Boolean     anim        True to animate the transition
     * @param   Function    cb          A callback to use when the animation
     *                                  completes
     * @return  void
    ...
    /**
     * Resizes Xlangdowbox to the appropriate height and width for the current
     * content.
     *
     * @param   Function    cb      A callback function to execute after the
     *                              resize completes
    ...
    /**
     * Calculates the dimensions for Xlangdowbox, taking into account the borders
     * and surrounding elements of #Xl-body.
     *
     * @param   Number      height      The content height
     * @param   Number      width       The content width
     * @param   Boolean     resizable   True if the content is able to be
    ...
    return {
         ...
         * @public
         */
        markup: '<div id="Xl-container">' +
                    '<div id="Xl-overlay"></div>' +
                    '<div id="Xl-wrapper">' +
                        '<div id="Xl-title">' +
                            '<div id="Xl-title-inner"></div>' +
                        '</div>' +
                        '<div id="Xl-body">' +
                            '<div id="Xl-body-inner"></div>' +
                            '<div id="Xl-loading">' +
                                '<a onclick="Xlangdowbox.close()">{cancel}</a>' +
                            '</div>' +
                        '</div>' +
                        '<div id="Xl-info">' +
                            '<div id="Xl-info-inner">' +
                                '<div id="Xl-counter"></div>' +
                                '<div id="Xl-nav">' +
                                    '<a id="Xl-nav-close" title="{close}" onclick="Xlangdowbox.close()"></a>' +
                                    '<a id="Xl-nav-next" title="{next}" onclick="Xlangdowbox.next()"></a>' +
                                    '<a id="Xl-nav-play" title="{play}" onclick="Xlangdowbox.play()"></a>' +
                                    '<a id="Xl-nav-pause" title="{pause}" onclick="Xlangdowbox.pause()"></a>' +
                                    '<a id="Xl-nav-previous" title="{previous}" onclick="Xlangdowbox.previous()"></a>' +
                                '</div>' +
        ...
        init: function(){
            ...
            if(S.client.isIE6){
                // trigger hasLayout on Xl-body
                U.get('Xl-body').style.zoom = 1;
         ...
         */
        bodyEl: function(){
            return U.get('Xl-body-inner');
        },

        /**
         * Called when Xlangdowbox opens from an inactive state.
         *
        ...
        onOpen: function(h, w, cb){
            U.get('Xl-container').style.display = 'block';
        ...
        onLoad: function(content, change, cb){
            ...
            hideBars(change, function(){ // if changing, animate the bars transition
                ...
                // if opening, clear #Xl-wrapper display
                if(!change) U.get('Xl-wrapper').style.display = '';
        ...
        /**
         * Called when Xlangdowbox is closed.
         *
         * @return  void
         * @public
         */
